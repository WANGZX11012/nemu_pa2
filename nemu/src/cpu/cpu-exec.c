...
#include "itbuf.h" //new for itrace
...
#ifdef CONFIG_ITRACE
  itbuf_p(_this->pc, _this->logbuf); //只在itrace启动时push
#endif
...
static void execute(uint64_t n)
{
  ...
  for (;n > 0; n --)
  {
    ...
    if (++dev_update_cnt >= 3800000) { dev_update_cnt = 0; device_update(); }
#endif
  }
}
    ...
    case NEMU_END: case NEMU_ABORT:
      Log("nemu: %s at pc = " FMT_WORD,
          (nemu_state.state == NEMU_ABORT ? ANSI_FMT("ABORT", ANSI_FG_RED) :
           (nemu_state.halt_ret == 0 ? ANSI_FMT("HIT GOOD TRAP", ANSI_FG_GREEN) ://ebreak给这些赋值了
