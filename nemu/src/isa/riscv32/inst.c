...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("0000001 ????? ????? 011 ????? 01100 11", mulhu  , R, R(rd) = my_mulhu(src1, src1) ); //test itrace
  
  INSTPAT("0000000 00001 00000 000 00000 11100 11", ebreak , N, NEMUTRAP(s->pc, R(10))); // R(10) is $a0
  
  INSTPAT("??????? ????? ????? 000 ????? 00100 11", addi   , I, R(rd) = src1 + imm ); //addi
  // INSTPAT("??????? ????? ????? 000 ????? 00100 11", addi   , I, R(rd) = src1 - imm ); //test for itrace
  ...
  #ifdef CONFIG_FTRACE
  { 
    int rs1_num = (s->isa.inst >> 15) & 0x1f;//前面decode是局部变量 不能直接用
    if (rd == 1) ftrace_call(s->pc, s->dnpc); 
